...
test.group('Router | add', () => {
  ...
  test('raise error when route name is duplicate', ({ assert }) => {
    ...
    assert.throws(
      fn,
      'A route with name "home" already exists. It may happen when two routes use the same controller, so make sure to give explicit names to these routes'
    )
  })
...
test.group('Router | toJSON', () => {
  test('convert routes with different style of handlers to JSON', ({ assert }) => {
    const router = new RouterFactory().create()

    function handler() {}
    const CategoriesControllerRef = async () => {
      return {
        default: class CategoriesController {
          async index() {}
        },
      }
    }
      ...
      root: [
        ...
        },
        {
          ...
          handler: {
            ...
            reference: 'PostsController.index',
          },
        },
        {
          pattern: '/comments',
          handler: {
            name: 'CommentsController.index',
            reference: 'CommentsController.index',
          },
        },
        {
          pattern: '/categories',
          handler: {
            name: 'CategoriesControllerRef',
            reference: [CategoriesControllerRef, 'index'],
          },
        },
      ],
    })
  ...
  test('convert routes with global middleware to JSON', ({ assert }) => {
    const router = new RouterFactory().create()

    const AuthMiddlewareRef = async () => {
      return {
        default: class AuthMiddleware {
          async handle() {}
        },
      }
    }

    router.use([AuthMiddlewareRef])
    router.get('comments', 'CommentsController.index')
  ...
  test('convert routes with inline and middleware', ({ assert }) => {
    const router = new RouterFactory().create()
    ...
    const named = router.named({ auth: AuthMiddlewareRef })
    router
      .get('comments', 'CommentsController.index')
      .use(named.auth())
      .use(async function log() {})
      .use(async () => {})
